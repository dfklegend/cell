...
func testCall(ms *client.MailStation) {
	for i := 0; i < 10; i++ {
...
type CBFuncToTest func(*client.MailStation)
...
func Test_Normal(t *testing.T) {
	createAndTest(func(ms *client.MailStation) {
		// service not found
		ms.Call(fmt.Sprintf("s%v", rand.Intn(1)), "entry1.join",
			&test.InMsg{Abc: fmt.Sprintf("%v", rand.Intn(100))},
			common.ReqWithCB(reflect.ValueOf(func(result *test.OutMsg, e error) {
...
func callEcho(ms *client.MailStation, str string) {	
