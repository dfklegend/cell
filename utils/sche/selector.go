...
func NewMultiSelector() *MultiSelector {
	...
	// 加入5s超时
	m.tickerTimeout = time.NewTicker(5*time.Second)
	m.AddSelector(NewFuncSelector(reflect.ValueOf(m.tickerTimeout.C),
...
func (s *MultiSelector) AddSelector(selector IChanSelector) {
	//fmt.Printf("AddSelector prelock\n")
	s.mutex.Lock()
	//fmt.Printf("AddSelector locked\n")
...
func (s *MultiSelector) HandleOnce() {
	...
	// 是否直接删除?
	if !recvOk {
		s.handleDeadChan(data)
	}
}

func (s *MultiSelector) tryMakeCases() {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *MultiSelector) handleDeadChan(data *SelectorData) {
	s.mutex.Lock()
	defer s.mutex.Unlock()
